...
if argv_len < 2 or argvs[1] in ("h", "-h", "help", "-help") :
    ...
    print ("")
    print ("Usage：")
    print ("chr2aligntime.py  sample1:pos-pos   sample2:pos-pos             target_folder[default chr2aligntime/{target1}_{target2}]     -kmer_len [default 41]  -seq2reverse [default no, otherwise write yes]")
    print('target format example：PN40024:Chr1:15154005-15285000   PN40024:Chr1:15285000-15435737   ')
    print (" ")
    sys.exit()
argv1=argvs[1]
...
subprocess.run([f"mkdir -p chr2aligntime"], shell=True)


#python chr2aligntime.py V124.hap2:Chr4:13656514-13726514 V124.hap2:Chr4:13726514-13799203 49b
#python chr2aligntime.py V124.hap2:Chr4:13478189-13534265 V124.hap2:Chr4:13534265-13590340 49a
...
target2=sys.argv[2]
if argv_len==3:
    output_dir=f'chr2aligntime/{target1}_{target2}'
else:    
    output_dir=sys.argv[3]  #'chr2aligntime/'+
current_dir = os.getcwd()

if "kmer_len"  in args_dict: #print("Missing input fasta file");sys.exit()
    kmer_len = int(args_dict["kmer_len"])
else:
    kmer_len=41

if "seq2reverse"  in args_dict: #print("Missing input fasta file");sys.exit()
    seq2reverse = args_dict["seq2reverse"]
else:
    seq2reverse='yes'
...
if 'step0'=='step0':
    ...
    sample1=        target1.split(':')[0]
    chromosome1=    target1.split(':')[1]
    pos1=           target1.split(':')[2]
    sample2=        target2.split(':')[0]
    chromosome2=    target2.split(':')[1]
    pos2=           target2.split(':')[2]
    ...
    subprocess.run([f"samtools faidx /home/lain/aaa_data/run0/new_work_dir/chr2EDTA/0_prepare/{sample1}/{sample1}.fasta {chromosome1}:{pos1} > {output_dir}/seq1.fa  "], shell=True)  
    subprocess.run([f"samtools faidx /home/lain/aaa_data/run0/new_work_dir/chr2EDTA/0_prepare/{sample2}/{sample2}.fasta {chromosome2}:{pos2} > {output_dir}/seq2.fa  "], shell=True) 
    
    seq1=''
    with open(f"{output_dir}/seq1.fa",'r') as f:
        next(f)
        for line in f:
            seq1+=line.strip()
    ...
    seq2=''
    with open(f"{output_dir}/seq2.fa",'r') as f:
        next(f)
        for line in f:
            seq2+=line.strip()    
    ...
    if seq2reverse=='yes':
        ...
        seq2=seq2
    else:print('error, -seq2reverse = yes/no');sys.exit()    
    seq2_len=len(seq2)
...
if 'step2'=='step2':
    #print('step1__Use kmers to find all kmer41 points')
    #print('step2__Extend and filter the points')
    #print('step3__Sort by length, then sequentially calculate the nearest points for extension')
    ...
    if 'step2.1'=='step2.1':
        print('Use kmers to find all kmer points')
        from collections import defaultdict
        import matplotlib.pyplot as plt
...
"""
    ...
    if 'step2.2'=='step2.2':
        ...
        print('Manually analyze continuous points, mimicking the custom script slash of moddotplot')
        #Point pool 
        ...
        x_gap_max=100         #x_ydot_neighbor is also related to this, after x moves right by +1, y can only have at most one point within ±5,      #Horizontal search
        y_gap_max=100                                                                                   #Vertical search
        extend_tolerance=10000
        ### Calculate diagonal upward
        with open(f"{output_dir}/kmer_matches.up_lines",'w') as fffff:
            ...
            for x in x_list:
                ...
                for y in y_list:
                            ...
                            while i2<y_gap_max:
                                ...
                                if x_tmp in dict3_x_y_z:
                                    if y_tmp in dict3_x_y_z[x_tmp]:
                                        ...
                                        good_mark='yes';break
                        ##If the next point is collected, continue with the next point as the starting point            
                        if good_mark=='yes': 
                    ...
                    #sys.exit()
                    ###Determine the number of points in the currently collected point sequence, slope, and fit. [[x1,y1],[x2,y2],...], calculate the slope and fit of the fitted line for all points
                    ...
                    if 1==1:#tmp_dot_list_len>30:
                        ...
                        all_start_y=tmp_dot_list[0][1];                 seq2_start=all_start_y     #seq2_start     #+(unit_len-1) is because the simplified start position is used
...
"""
    ...
    if 'step2.3'=='step2.3':
        ...
        def get_slople_R2(dot_list):
            ...
            y_pred = slope * np_x + intercept  # Calculate R^2 goodness of fit
        ...
        with open(f"{output_dir}/kmer_matches.up_lines2",'w') as f:
            ...
            for one_line in lines:
                ...
                for one_line in lines:
                    ...
                    ##Filter out already included intervals
...
"""
    ...
    if 'step2.31'=='step2.31':       
        ##For subsequent segment mafft, if there are 300k+ continuous segments, they cannot be aligned due to memory issues. I will manually split them into roughly 100k bp small segments
    ...
    if 'step2.4'=='step2.4':        
        ...
        def run_step(one_index):     #--op 4 
            ...
            with open(f'{output_dir}/segments/{one_index}.mafft','w') as f:
                f.write(f'>seq1_part\n{seq1_mafft}\n>seq2_part\n{seq2_mafft}')    
            #Separate by indel, keep continuous segments longer than 100bp
